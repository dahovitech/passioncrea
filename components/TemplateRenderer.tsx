...
const TemplateRenderer: React.FC<Props> = ({ data, mandateLogoUrl, onUpdateData }) => {
  const { 
    ...
    imageUrl, imageZoom, imagePosX, imagePosY, backgroundImageUrl, accentColor, textColor, backgroundColor, selectedLogo, primaryFont, birthdayTitleFont, partners, socialIcons,
    ...
    if (variant === 2) return (
      <div className="flex-grow pt-56 px-12 flex flex-col">
        <div className="border-l-[25px] pl-10 mb-10" style={{ borderColor: accentColor }}>
          <p className="text-3xl font-bold uppercase tracking-[0.4em]" style={{ color: '#C6A967' }}>{primaryText || "Planning"}</p>
        ...
        <div className="grid grid-cols-2 gap-6">
          {agendaItems.map((item) => (
            <div key={item.id} className="flex items-center gap-5 p-5 bg-white rounded-[35px] shadow-2xl border-t-4" style={{ borderTopColor: accentColor }}>
              <div className="text-4xl font-bold" style={{ color: '#C6A967' }}>{item.day}</div>
              <div>
  ...
  const renderFormation = () => {
    if (variant === 1) return (
      <div className="flex-grow pt-56 flex flex-col">
    ...
    if (variant === 2) return (
      <div className="flex-grow pt-56 px-12 flex items-center justify-between gap-12">
  ...
  const renderBirthday = () => {
    ...
    if (variant === 1) return (
      <div className="flex-grow pt-56 px-20 flex items-center">
        <div className="flex items-center justify-between w-full gap-4">
          <div className="text-left space-y-4 min-w-0" style={{ flex: '1 1 400px' }}>
            <h2 className="italic leading-none" style={{ fontFamily: birthdayTitleFont, fontSize: 'clamp(2rem, 4vw, 4.5rem)', color: '#C6A967', textShadow: '3px 3px 0px rgba(0,0,0,0.1)', whiteSpace: 'nowrap' }}>HAPPY<br/>BIRTHDAY</h2>
            <p className="text-4xl md:text-5xl font-black tracking-tighter" style={{ color: nameColor || textColor }}>{name}</p>
            <div className="pl-2">
              <p className="text-xl md:text-2xl font-black tracking-[0.4em]" style={{ color: roleColor || '#005596' }}>{role}</p>
              {secondaryText && <p className="text-lg font-bold italic mt-2 opacity-70 truncate" style={{ color: textColor }}>{secondaryText}</p>}
            </div>
          </div>
          <div className="relative shrink-0">
            ...
            <SubjectImage className="w-[280px] h-[280px] md:w-[320px] md:h-[320px] rounded-full border-[15px] border-white shadow-xl relative z-[10]" />
          </div>
        </div>
      </div>
    );
    ...
    if (variant === 2) return (
      <div className="flex-grow flex flex-col items-center pt-56 px-12">
        <h2 className="tracking-widest mb-8" style={{ fontFamily: birthdayTitleFont, fontSize: 'clamp(2rem, 4vw, 4rem)', color: '#C6A967', textShadow: '4px 4px 0px rgba(0,0,0,0.15)', whiteSpace: 'nowrap' }}>HAPPY BIRTHDAY</h2>
        <div className="flex items-center gap-20">
          <div className="flex flex-col items-center shrink-0 transform -rotate-2">
            <div className="w-[500px] h-[500px] rounded-[100px] border-[20px] border-white shadow-2xl overflow-hidden">
              <SubjectImage />
            </div>
          </div>
      ...
      <div className="flex-grow pt-56 px-12 flex flex-col items-center justify-center text-center relative overflow-hidden">
         <div className="flex items-center gap-20 relative z-10">
            <div className="text-left space-y-5">
               <h2 className="leading-none" style={{ fontFamily: birthdayTitleFont, fontSize: 'clamp(2rem, 4vw, 4.5rem)', color: '#C6A967', whiteSpace: 'nowrap' }}>HAPPY<br/>BIRTHDAY</h2>
               <h2 className="text-6xl font-black tracking-tighter leading-none" style={{ color: nameColor || textColor }}>{name}</h2>
               <div className="flex items-center gap-4">
                  <div className="h-1 w-16 rounded-full" style={{ backgroundColor: accentColor }}></div>
                  <p className="text-xl font-black tracking-[0.4em]" style={{ color: roleColor || '#005596' }}>{role}</p>
               </div>
               <p className="text-4xl font-black italic" style={{ color: dateColor || textColor }}>{date}</p>
               {secondaryText && <p className="text-xl font-bold opacity-60 mt-2" style={{ color: textColor }}>{secondaryText}</p>}
            </div>
            <SubjectImage className="w-[400px] h-[400px] rounded-full border-[20px] border-white shadow-2xl shrink-0" style={{ borderColor: accentColor }} />
         </div>
      </div>
    );
  };
  ...
  const renderMotivation = () => {
    ...
    if (variant === 2) return (
      <div className="flex-grow flex flex-col pt-56 px-12">
